...
import { accounts as seedAccounts, guardrails, initialAuditLog } from "@/lib/demoData";
import {
  approveAccount,
  calculateMetrics,
  filterAccounts,
...
const filters = ["All", "Needs review", "Ready to send", "Payment plan", "Escalation hold", "Approved"];
const sortOptions = [
  ["risk", "Risk"],
  ["balance", "Balance"],
  ["age", "Age"],
...
export default function CollectionsConsole() {
  const [accountState, setAccountState] = useState(() => seedAccountState(seedAccounts));
  const [selectedId, setSelectedId] = useState(accountState[0].id);
  ...
  const visibleAccounts = useMemo(() => {
    return filterAccounts(accountState, { filter, search, sort });
  }, [accountState, filter, search, sort]);

  const selected = accountState.find((account) => account.id === selectedId) ?? accountState[0];
  const { totalBalance, expectedRecovery, reviewCount, highRiskCount } = calculateMetrics(accountState);

  function approveAction() {
    if (selected.approvedAction) {
      return;
    }
      ...
      <section className="topbar" aria-label="Workspace summary">
        <div>
          ...
          <p className="subtitle">
            A human-reviewed workflow for overdue B2B accounts: AI drafts, reason codes, approval queue and audit trail.
          </p>
        </div>
        <button className="primaryButton" onClick={simulateAgentRun} disabled={isRunningBatch} type="button">
      ...
      <section className="workArea">
        <div className="queuePanel">
          ...
          <div className="filterTabs" role="tablist" aria-label="Account status filters">
            {filters.map((item) => (
              <button
                ...
                onClick={() => {
                  setFilter(item);
                  const nextVisible = filterAccounts(accountState, { filter: item, search, sort });
          ...
          <div className="queueTools" aria-label="Queue search and sort controls">
            <label>
              <span>Search</span>
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Company, owner, reason" />
            </label>
            <label>
              <span>Sort</span>
          ...
          <div className="accountList">
            {visibleAccounts.map((account) => {
              const approved = account.approvedAction;
              return (
                <button
                  key={account.id}
                ...
                >
                  <span>
                    ...
                    <small>{account.id} · {account.daysPastDue} days past due</small>
                  </span>
                  <span className="rowRight">
                    <b>{money.format(account.balance)}</b>
                    <em className={`statusPill ${statusClass(account.status)}`}>
                      {approved ? "Approved" : account.status}
                    </em>
                  </span>
                </button>
              );
            })}
            {visibleAccounts.length === 0 ? <p className="emptyState">No accounts match this queue view.</p> : null}
          </div>
        </div>
        ...
        <div className="detailPanel">
          ...
          <div className="actionRow">
            <button className="primaryButton" onClick={approveAction} disabled={selected.approvedAction} type="button">
            ...
            <div className="auditList">
              {auditLog.slice(0, 7).map((item, index) => (
                <article key={`${item.time}-${item.account}-${index}`}>
                  ...
                  <p>{item.action}</p>
                  <small>{item.actor} · {item.outcome}</small>
                </article>
              ))}
            </div>
          </section>
